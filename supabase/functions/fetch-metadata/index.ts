...
function handleAdidas(url: string) {
  ...
  if (sku) {
    ...
    let nameSlug = urlParts[urlParts.length - 2] || "adidas-product";
...
async function handleVictoriasSecret(url: string) {
  ...
  try {
    // 1. استخراج معرف المنتج واللون من الرابط لضمان الدقة
    const urlObj = new URL(url);
    const genericId = urlObj.searchParams.get("genericId"); // 11276485
    const choice = urlObj.searchParams.get("choice"); // 7D6T

    // الكود المدمج الذي نبحث عنه في الصورة (مثل 112764857D6T)
    const targetSku = genericId && choice ? `${genericId}${choice}` : "";

    // 2. استخدام Jina AI لقراءة الصفحة
    ...
    let title = titleMatch ? titleMatch[1].replace("Victoria's Secret", "").trim() : "VS Product";

    // 4. استخراج الصورة (الخدعة الذكية) 🧠
    let image = "";

    if (targetSku) {
      // نبحث عن صورة تحتوي على الكود المدمج (لنجلب اللون الصحيح)
    ...
    // إذا لم نجد صورة اللون المحدد، نأخذ أي صورة منتج (تبدأ بـ /p/)
    if (!image) {
...
async function handleJomashop(url: string) {
  ...
  try {
    ...
    let title = titleMatch ? titleMatch[1] : "Jomashop Product";
...
async function handleMacys(url: string) {
  ...
  try {
    ...
    let title = titleMatch
      ? titleMatch[1].replace(" - Macy's", "").replace("Created for Macy's", "").trim()
      : "Macy's Product";
...
async function handleIherb(url: string) {
  ...
  try {
    ...
    let titleMatch = text.match(/^Title:\s*(.+)$/m);
    let title = titleMatch ? titleMatch[1].replace("- iHerb", "").trim() : "iHerb Product";
  ...
  if (match) {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        return {
          title: `Nike - ${sku}`,
          description: "Imported Nike Sneaker",
