...
import { ChevronRight, Package, Calendar, AlertTriangle } from 'lucide-react';
...
interface OrderCardProps { order: Order; onUpdate: () => void; }
...
export function OrderCard({ order, onUpdate }: OrderCardProps) {
  ...
  const handleReportIssue = async () => {
    ...
    const { error: disputeError } = await supabase.from('disputes').insert({
      ...
      issue_type: issueType, description: issueDescription,
    });
    if (!disputeError) {
      await supabase.from('orders').update({ status: 'under_investigation' }).eq('id', order.id);
    }
    setIsSubmitting(false);
    if (disputeError) { toast.error('Failed to submit report'); return; }
  ...
  const totalIqd = totalAmount > 0
    ? Math.round(totalAmount * settings.usdToIqd).toLocaleString() + ' IQD'
    : null;

  // #16 — اقتطاع اسم المنتج الطويل
  const title = order.product_title || 'Product Order';
    ...
    <div className="p-4 sm:p-6 rounded-2xl bg-card border border-border hover:shadow-soft transition-shadow">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-xl bg-muted overflow-hidden">
          {order.product_image ? (
            <img src={order.product_image} alt={shortTitle}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
            <div>
              {totalAmount > 0 ? (
                <>
                  <span className="text-lg font-display font-bold">${totalAmount.toFixed(2)}</span>
                  {totalIqd && (
                    <span className="text-xs text-muted-foreground ml-2">{totalIqd}</span>
                  )}
                </>
              ) : (
                    ...
                    <div className="space-y-4 pt-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea placeholder="Describe the issue in detail..."
                          value={issueDescription} onChange={(e) => setIssueDescription(e.target.value)} rows={4} />
                      </div>
